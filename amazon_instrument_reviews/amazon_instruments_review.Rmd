...
reviews <- read_csv("amazon_instrument_reviews/amazon-music-reviews/Musical_instruments_reviews.csv") %>%
  ...
  mutate(five_stars = if_else(overall == 5, "yes", "no")) %>% 
...
by_month <- reviews %>% 
  ...
  summarize(
    ...
    pct_5 = mean(overall == 5),
    pct_lower = mean(overall != 5)
  ) %>% 
  ungroup()
...
I am not a statistician (yet) so I will make any definitive statements as to the statistical integrity of this model. I am not including for purely demonstrational purposes.
