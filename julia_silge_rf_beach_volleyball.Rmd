...
winners <- vb_parsed %>%
  ...
  # to identify wins when binded to losers data
  mutate(win = "win")
...
```{r}
vb_df %>%
  # create stat and value column where the .cols become values of the `stat` column and each value of those stats becomes the `value` column
...
```{r}
# best five models in terms of area under the curve
